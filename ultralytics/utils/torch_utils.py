...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            input_channels = 3
            first_layer = list(model.modules())[0]
            if isinstance(first_layer, torch.nn.Conv2d):
                input_channels = first_layer.in_channels
            stride = max(int(model.stride.max()), 32) if hasattr(model, 'stride') else 32
            im = torch.zeros((1, input_channels, stride, stride), device=next(model.parameters()).device)
            macs, params = thop.profile(deepcopy(model), inputs=(im,), verbose=False)
            flops = macs * 2
            flops = flops / 1E9
